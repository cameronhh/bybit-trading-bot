...
class THMPipeline:
    ...
    def run_pipeline(self):
        param_config = configparser.ConfigParser()
        param_config.read('strategies/configs/thm_config.ini')
...
if __name__ == "__main__":
    ...
    exchange = BybitExchange(test=True, api_key=api_key, private_key=api_secret)
    ...
    test_strategy = THMStrategy(
        state = int(param_config['DEFAULT']['state']),
        wt_open_long = float(param_config['DEFAULT']['wt_open_long']),
        wt_close_long = float(param_config['DEFAULT']['wt_close_long']),
        mfi_open_long = float(param_config['DEFAULT']['mfi_open_long']),
        wt_open_short = float(param_config['DEFAULT']['wt_open_short']),
        wt_close_short = float(param_config['DEFAULT']['wt_close_short']),
        mfi_open_short = float(param_config['DEFAULT']['mfi_open_short']),
    )

    test_strategy.load_klines(data=data)

    backtester = Backtester(strategy=test_strategy, stake_percent=0.03, pyramiding=20)
    backtester.print_report()
